...
public:
    fbuf(PHYSFS_File* file, std::size_t bufferSize = 2048)
...
ifstream::ifstream(const string& filename)
    : base_fstream(openWithMode(filename.c_str(), READ))
...
bool mount(const string& newDir, const string& mountPoint, bool appendToPath)
{
