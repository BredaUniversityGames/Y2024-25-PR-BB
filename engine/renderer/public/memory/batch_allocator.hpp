...
{
public:
    BatchAllocator(std::shared_ptr<GraphicsContext> context, const BufferCreation& sourceBufferParams);
    ~BatchAllocator();

    NON_MOVABLE(BatchAllocator);
    NON_COPYABLE(BatchAllocator);
    ...
    template <typename T>
    BatchAllocation Allocate(SingleTimeCommands& commands, const std::vector<T>& data)
    {
        return Allocate(commands, std::bit_cast<const byte*>(data.data()), data.size() * sizeof(T));
    }

    BatchAllocation Allocate(SingleTimeCommands& commands, const std::byte* data, size_t size);
    void Free(BatchAllocation offset);

private:
    ResourceHandle<Buffer> _buffer;
    std::shared_ptr<GraphicsContext> _context;
    VmaVirtualBlock _subAllocator {};
};
