...
void PresentationPass::RecordCommands(vk::CommandBuffer commandBuffer, MAYBE_UNUSED uint32_t currentFrame, const RenderSceneDescription& scene)
{
    ...
    util::TransitionImageLayout(commandBuffer, inputImage->Get(), inputImage->format,
        vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::eTransferSrcOptimal, 1, 0, 1);

    util::CopyImageToImage(commandBuffer, inputImage->Get(), _swapChain.GetImage(scene.targetSwapChainImageIndex), vk::Extent2D { inputImage->width, inputImage->height }, _swapChain.GetExtent());
