...
void BloomDownsamplePass::RecordCommands(vk::CommandBuffer commandBuffer, uint32_t currentFrame, const RenderSceneDescription& scene)
{
    ...
    {
        uint32_t targetMip = mip + 1;

        vk::RenderingAttachmentInfoKHR finalColorAttachmentInfo {
            .imageView = image->layerViews[0].mipViews[targetMip],
        ...
        mipBarrier.subresourceRange.baseMipLevel = targetMip; // Sync only next mip for reading
...
void BloomDownsamplePass::CreatPipeline()
{
    ...
    std::vector<std::byte> fragSpv = shader::ReadFile("shaders/bin/bloom_downsample.frag.spv");
