...
void GaussianBlurPass::RecordCommands(vk::CommandBuffer commandBuffer, uint32_t currentFrame, MAYBE_UNUSED const RenderSceneDescription& scene)
{
    ...
    util::TransitionImageLayout(commandBuffer, verticalTarget->Get(), verticalTarget->format, vk::ImageLayout::eUndefined, vk::ImageLayout::eColorAttachmentOptimal);
    ...
    for (uint32_t i = 0; i < blurPasses * 2; ++i)
    {
        ...
        if (i != 0)
        {
            ...
            util::TransitionImageLayout(commandBuffer, source->Get(), source->format, vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);

            // The first vertical pass, the target is already set up for color attachment
            if (i != 1)
            {
                util::TransitionImageLayout(commandBuffer, target->Get(), target->format, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageLayout::eColorAttachmentOptimal);
            }
        }
