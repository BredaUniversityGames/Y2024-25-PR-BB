...
void ShadowPass::DrawGeometry(vk::CommandBuffer commandBuffer, uint32_t currentFrame, const RenderSceneDescription& scene, bool prepass)
{
    ...
    if (scene.gpuScene->ShouldUpdateShadows() == true)
    {
        ...
        if (scene.gpuScene->StaticDrawCount() > 0)
        {
            ...
            vk::Buffer vertexBuffer = resources->BufferResourceManager().Access(scene.staticBatchBuffer->VertexBuffer())->Get();
            vk::Buffer indexBuffer = resources->BufferResourceManager().Access(scene.staticBatchBuffer->IndexBuffer())->Get();
            vk::Buffer indirectDrawBuffer = resources->BufferResourceManager().Access(_cameraBatch.StaticDraw().drawBuffer)->Get();
            vk::Buffer countBuffer = resources->BufferResourceManager().Access(_cameraBatch.StaticDraw().redirectBuffer)->Get();
...

    {
        ...
        if (scene.gpuScene->SkinnedDrawCount() > 0)
        {
            ...
            vk::Buffer countBuffer = resources->BufferResourceManager().Access(_cameraBatch.SkinnedDraw().redirectBuffer)->Get();
