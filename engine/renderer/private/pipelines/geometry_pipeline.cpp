...
GeometryPipeline::GeometryPipeline(const std::shared_ptr<GraphicsContext>& context, const GBuffers& gBuffers, const CameraBatch& cameraBatch)
    : _context(context)
    , _gBuffers(gBuffers)
...
void GeometryPipeline::DrawGeometry(vk::CommandBuffer commandBuffer, uint32_t currentFrame, const RenderSceneDescription& scene, bool prepass)
{
    ...
    if (scene.gpuScene->StaticDrawRange().count > 0)
    {
        ...
        commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, _staticPipelineLayout, 3, { scene.gpuScene->MainCameraBatch().RedirectBufferDescriptorSet() }, {});
        ...
        vk::Buffer countBuffer = _context->Resources()->BufferResourceManager().Access(_cameraBatch.RedirectBuffer())->buffer;
