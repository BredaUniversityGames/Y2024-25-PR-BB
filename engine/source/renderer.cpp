    ...
    , _bloomSettings(_brain)
{
    ...
    _particlePipeline = std::make_unique<ParticlePipeline>(_brain, *_gBuffers, _hdrTarget, *_camera, *_swapChain);
...
void Renderer::RecordCommandBuffer(const vk::CommandBuffer& commandBuffer, uint32_t swapChainImageIndex, float deltaTime)
{
    ...
    _particlePipeline->RecordCommands(commandBuffer, _currentFrame, sceneDescription, *_ecs, deltaTime);
